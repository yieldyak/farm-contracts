...
export async function fundMiniYakSwap(yakAvaxPairAddress: string, miniYakSwapAddress: string, miniYakAddress: string, yakAddress: string) {
    ...
    await miniYakToken.approve(miniYakSwap.address, ethers.constants.MaxUint256);    await fundTokenFromAVAXPair(yakAvaxPairAddress, BigNumber.from("10000000000000000000"), mySigner)
